...
def create_data_batches(X_t, X_s, Y, sequence_length, batch_size):
    ...
    for i in range(len(Y) - sequence_length + 1):
        data.append([X_t[i:i + sequence_length], X_s[i:i + sequence_length]])
...
train_data, train_target = create_data_batches(X_t_values, X_s_values, Y_values, sequence_length, batch_size)
